...
(function () {
	...
	kintone.events.on(eventsShow, function (event) {
		...
		if (('app.record.create.show').indexOf(event.type) >= 0){
			record['BuyerCd']['value'] = "";
		}
		record['BuyerCd']['disabled'] = true;
	...
	kintone.events.on(eventAdd, function(event) {
		...
		// サービス初期化
		var buyerService = new BuyerService(record);
		
		// API実行
		if (buyerService.getBuyerCd()){
			// 採番したBuyerCdを設定
			record['BuyerCd']['value'] = buyerService.getAutoBuyerCd();
		}else {
			event.error = buyerService.getMessage();
		}
