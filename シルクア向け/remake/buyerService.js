...
BuyerService = function(record) {
	this._init(record);
};
BuyerService.prototype = {
	...
	_init: function(record) {
		...
		this.strBuyerCd =  this.record['BuyerCd']['value'];
		
		// クリエリー作成
		this.query = 'AreaKbn in ("' + this.strAreaKbn + '") order by BuyerCd limit 1';
	...
	/* 購入者コードの採番                    */
	/***************************************/
	getBuyerCd: function(keyVal) {
		if (this._getRecords()){
			// 新規BuyerCdを取得
			if (this._getMaxNumber('BuyerCd')){
	...
	_getMaxNumber: function(keyVal) {
		...
		if (obj.records[0] != null){
			try{
				...
				this.recNo = parseInt(strGetVal.slice(-6),10) +1;
	...
	getAutoBuyerCd: function() {
		return getAreaCd(this.strAreaKbn) + ('000000' + this.recNo).slice(-6);
	},
	...
	_getQueryObj: function(records) {
	    ...
	    for (var i = 0, l = records.length; i < l; i++) {
    		...
    		partObj["record"] = {BuyerCd:{value:record['BuyerCd']['value']}};
