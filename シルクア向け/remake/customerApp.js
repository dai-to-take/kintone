...
(function () {
	...
	kintone.events.on(eventsShow, function (event) {
		...
		if (('app.record.create.show').indexOf(event.type) >= 0){
			record['CustomerCd']['value'] = "";
		}
		record['CustomerCd']['disabled'] = true;
	...
	kintone.events.on(eventsEdit, function (event) {
		var record = event.record;
		record['CustomerKbn']['disabled'] = true;
	...
	kintone.events.on(eventAdd, function(event) {
		...
		// サービス初期化
		var customerService = new CustomerService(record);
		
		// API実行
		if (customerService.getCustomerCd()){
			// 採番したCustomerCdを設定
			record['CustomerCd']['value'] = customerService.getAutoCustomerCd();
		}else {
			event.error = customerService.getMessage();
		}
