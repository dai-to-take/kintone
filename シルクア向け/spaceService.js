...
SpaceService.prototype = {
	...
	_init: function(record) {
		...
		this.strSpaceCd =  this.record['SpaceCd']['value'];
	...
	/* 場所コードの採番                    */
	/***************************************/
	getSpaceCd: function(keyVal) {
		if (this._getRecords()){
			// 新規SpaceCdを取得
			if (this._getMaxNumber('SpaceCd')){
	...
	_getQueryObj: function(records) {
	    ...
	    for (var i = 0, l = records.length; i < l; i++) {
    		...
    		partObj["record"] = {SpaceCd:{value:record['SpaceCd']['value']}};
