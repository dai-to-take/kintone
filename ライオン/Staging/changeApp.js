...
(function() {
	...
	kintone.events.on(eventsShow, function (event) {
		...
		// 共通
		record['ChangeNumber']['disabled'] = true;
		
		// アクション別
		switch (true) {
			case ('app.record.create.show').indexOf(event.type) >= 0:
				record['ChangeNumber']['value'] = "";
	...
	kintone.events.on(eventAdd, function(event) {
        var record = event.record;
		var autoChangeNumber = ""

		// サービス初期化
		var changeService = new ChangeService(record);

		// 伝票番号の採番
		if (changeService.getChangeNumber()){
			// 採番したReturnNumberを設定
			autoChangeNumber  = changeService.getAutoChangeNumber();
			record['ChangeNumber']['value'] = autoChangeNumber;
		} else {
			event.error = changeService.getMessage();
			return event;
		}
		
		// 委託以外の商品コードがあるとき、状態区分に返品が入力されていないかチェック
		if (! changeService.conditionKbnCheck()) {
			event.error = changeService.getMessage();
			return event;
		}
		
		// 関連情報登録（移動履歴、商品マスタ、在庫更新）
		if (! changeService.setRelationInfo(autoChangeNumber)) {
			event.error = changeService.getMessage();
			return event;
		}
		return event;
