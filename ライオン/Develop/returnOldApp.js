...
(function() {
	...
	kintone.events.on(eventAdd, function(event) {
		...
		// サービス初期化
		var returnOldService = new ReturnOldService(record);

		// 伝票番号の採番
		if (returnOldService.getReturnNumber()){
			// 採番したReturnNumberを設定
			autoReturnNumber  = returnOldService.getAutoReturnNumber();
			record['ReturnNumber']['value'] = autoReturnNumber;
		} else {
			event.error = returnOldService.getMessage();
			return event;
		}
		
		// 関連情報登録（移動履歴、商品マスタ、在庫更新）
		if (! returnOldService.setRelationInfo(autoReturnNumber)) {
